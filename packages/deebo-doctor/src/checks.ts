  ...
  async check() {
    ...
    try {
      const nodePath = execSync(
        isWindows ? 'cmd.exe /c where node.exe' : 'which node'
      ).toString().trim().split('\n')[0];
      results.push({
        name: 'node',
        status: 'pass',
    ...
    try {
      const npmPath = execSync(
        isWindows ? 'cmd.exe /c where npm.cmd' : 'which npm'
      ).toString().trim().split('\n')[0];
      results.push({
        name: 'npm',
        status: 'pass',
    ...
    try {
      const gitPath = execSync(
        isWindows ? 'cmd.exe /c where git.exe' : 'which git'
      ).toString().trim().split('\n')[0];
      results.push({
        name: 'git',
        status: 'pass',
    ...
    try {
      const rgPath = execSync(
        isWindows ? 'cmd.exe /c where rg.exe' : 'which rg'
      ).toString().trim().split('\n')[0];
      results.push({
        name: 'ripgrep',
        status: 'pass',
    ...
    const missingPaths = expectedPaths.filter(expected => 
      !paths.some(p => p.toLowerCase().includes(expected.toLowerCase()))
    );
